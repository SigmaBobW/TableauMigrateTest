...
_Power BI visual -> Sigma workbook element kind + ROLE CLASS. `source` is the coarse kind token the builder consumes (rec['sigma_kind']); `sigma` is the Sigma element kind; `pbi_visual_types` enumerates the raw PBI visualTypes feeding that token; `role_class` (control|kpi|chart|table|text|image|decoration|unsupported) says what the visual DOES, so the coverage gate can tell a functional loss (a lost slicer = the page lost its filter) from a cosmetic one (a lost decorative shape) from a DIFFERENT cosmetic one that still builds a real element when it can (a logo/banner `image`, which — unlike `decoration` — is never routed to a bare 'nothing built' skip; PbiVizKind.functional? still treats it as non-functional). Both the Ruby builder AND the Python extractors resolve through this one file via lib/pbi_viz_kind.{rb,py} — the previously-duplicated Python VISUAL_KIND dict is gone, so the two maps can no longer drift. Third-party/custom visuals live in custom-visual.json. NOTE 2026-08-26: Sigma now has NATIVE treemap-chart, sankey-chart, funnel-chart and gauge-chart (live-verified against a real org). The rows below still route treemap/funnel into `bar`, leave sankeyDiagram in `unsupported` (dropped entirely), and map gauge to `progress`. Re-routing is tracked follow-up work: it requires the builder to emit each kind's own pointer (treemap `category`, sankey `stages`+`value`, funnel `stage`+`series`, gauge `value`) -- changing only the `sigma` token here would emit an invalid element. See sigma-workbooks reference/specification/charts.md for the verified shapes._
...
| | | | | _PBI gauge -> native Sigma progress with shape:ring and mode:value. Value and optional min/max/target roles become progress formula strings. Sigma's ring is circular rather than Power BI's semicircle, so shape fidelity still requires visual review, but this is no longer a KPI fallback. A native gauge-chart NOW EXISTS (2026-08-26) -- still mapped to progress here; re-route pending._ |
...
| | | | | _These previously became bar charts. A bar chart of a decomposition tree is not an approximation, it is a wrong answer — hence role_class unsupported, reported honestly. sankeyDiagram: a native sankey-chart NOW EXISTS (2026-08-26) -- this row still drops it; re-route pending._ |
